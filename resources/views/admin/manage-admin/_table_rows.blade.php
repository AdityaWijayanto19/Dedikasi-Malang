    ...
    <tr>
        <td class="px-6 py-4 whitespace-nowrap">
            @if($item->avatar)
                <img class="h-10 w-16 object-cover rounded"
                    src="{{ Str::startsWith($item->avatar, 'http') ? $item->avatar : asset('storage/' . $item->avatar) }}"
                    alt="{{ $item->name }}">
            @else
                <div class="h-10 w-16 bg-gray-200 rounded flex items-center justify-center text-xs text-gray-500">No Image</div>
            @endif
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 max-w-sm" title="{{ $item->batch }}">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <span @class([
                ...
                'bg-green-100 text-green-800' => $item->status == \App\Enums\UserStatus::Active,
                'bg-gray-200 text-gray-800' => $item->status == \App\Enums\UserStatus::Block,
            ])>
                {{ $item->status->label() }}
            </span>
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center space-x-2">
            <a href="{{ route('admin.manage-admin.edit', $item->id) }}"
            ...
            <form action="{{ route('admin.manage-admin.destroy', $item->id) }}" method="POST" class="inline-block"
                @click.prevent="$dispatch('open-delete-modal', { action: '{{ route('admin.manage-admin.destroy', $item->id) }}',
                                        title: 'Hapus admin',
                                        message: 'Apakah Anda yakin ingin menghapus admin \'{{ Str::limit($item->batch, 30) }}\'? Data ini tidak dapat dikembalikan.'
                                    })">
                @csrf
                @method('DELETE')
        ...
        <td colspan="7" class="px-6 py-10 text-center">
            ...
            <div class="flex flex-col items-center justify-center space-y-4">
                ...
                <p class="text-sm text-gray-500 max-w-sm">
                    Tambahkan admin baru untuk mulai mengisi tabel ini. Semua data admin yang Anda buat akan muncul di
                    sini.
                </p>
            </div>
