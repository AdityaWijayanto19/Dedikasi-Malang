    ...
    <tr>
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <span @class([
                ...
                'bg-gray-200 text-gray-800' => $item->status == \App\Enums\StatusPostingan::Draft,
            ])>
                {{ $item->status->label() }}
            </span>
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center space-x-2">
            ...
            <form action="{{ route('admin.donasi.destroy', $item->id) }}" method="POST" class="inline-block" @click.prevent="$dispatch('open-delete-modal', { action: '{{ route('admin.donasi.destroy', $item->id) }}',
                                                        title: 'Hapus donasi',
                                                        message: 'Apakah Anda yakin ingin menghapus donasi \'{{ Str::limit($item->title, 30) }}\'? Data ini tidak dapat dikembalikan.'
                                                    })">
                @csrf
                @method('DELETE')
        ...
        <td colspan="7" class="px-6 py-10 text-center">
            ...
            <div class="flex flex-col items-center justify-center space-y-4">
                ...
                <h3 class="text-xl font-semibold text-gray-700">
                    Belum Ada donasi yang Ditambahkan
                </h3>

                <p class="text-sm text-gray-500 max-w-sm">
                    Tambahkan donasi baru untuk mulai mengisi tabel ini. Semua data donasi yang Anda buat akan muncul di
                    sini.
                </p>
            </div>
