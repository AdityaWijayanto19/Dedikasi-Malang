...
<form action="{{ $isEdit ? route('admin.kegiatan.update', $kegiatan->id) : route('admin.kegiatan.store') }}"
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="batch" id="batch"
                        value="{{ old('batch', $isEdit ? $kegiatan->batch : '') }}"
                        ...
                        placeholder="Contoh: Dedikasi Malang 1">
                </div>

                <div>
                    <label for="judul" class="block text-sm font-medium text-gray-700">Judul</label>
                    <input type="text" name="title" id="title"
                        value="{{ old('title', $isEdit ? $kegiatan->title : '') }}"
                        class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 focus:border-primary-yellow focus:ring focus:ring-primary-yellow/50 transition duration-150"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="tanggal" id="tanggal"
                        value="{{ old('tanggal', $isEdit ? $kegiatan->tanggal : '') }}"
                        class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 focus:border-primary-yellow focus:ring focus:ring-primary-yellow/50 transition duration-150"
                        placeholder="Contoh: 27 Oktober 2025">
                </div>
                <div>
                    <label for="lokasi" class="block text-sm font-medium text-gray-700">Lokasi</label>
                    <input type="text" name="lokasi" id="lokasi"
                        value="{{ old('lokasi', $isEdit ? $kegiatan->lokasi : '') }}"
                        class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 focus:border-primary-yellow focus:ring focus:ring-primary-yellow/50 transition duration-150"
                        placeholder="Contoh: Jalan Merdeka No. 45, RT 02/RW 03 ">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4"
                    class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 focus:border-primary-yellow focus:ring focus:ring-primary-yellow/50 transition duration-150"
                    placeholder="Jelaskan secara rinci mengenai kegiatan">{{ old('deskripsi', $isEdit ? $kegiatan->deskripsi : '') }}</textarea>
            </div>
            ...
            <div>
                ...
                <div id="file-drop-area"
                    ...
                    <div id="preview-container"
                        class="absolute inset-0 p-2 @unless($isEdit && $kegiatan->gambar) hidden @endunless">
                        @if ($isEdit && $kegiatan->gambar)
                            @php
                                $imageUrl = Str::startsWith($kegiatan->gambar, 'http') ? $kegiatan->gambar : asset('storage/' . $kegiatan->gambar);
                            @endphp
                    ...
                    <div id="upload-prompt"
                        class="absolute inset-0 flex flex-col items-center justify-center text-center p-4 @if(isset($kegiatan) && $kegiatan->gambar) opacity-0 hover:opacity-100 bg-black/50 text-white transition-opacity @endif">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="status" name="status"
                        class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 focus:border-primary-yellow focus:ring focus:ring-primary-yellow/50 transition duration-150">
                        @php
                            $currentStatusValue = old('status', $isEdit ? $kegiatan->status->value : null);
                        @endphp
                ...
                <div>
                    <label for="link" class="block text-sm font-medium text-gray-700">Link Dokumentasi
                        <x-tooltip>
